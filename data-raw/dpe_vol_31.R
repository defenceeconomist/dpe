...
for(i in 1:nrow(volume_pdf)){
  od$download_file(srcid = volume_pdf$id[i], dest = file.path(tmp_dir, volume_pdf$name[i]), overwrite = TRUE)
}
...
dpe_vol_31 <- crossref_pdf
usethis::use_data(dpe_vol_31)
